function Modis_Stacker_Plot_Data_Age(Data_stacked_age,end_day, Center_date, Date,geo,print_fig,img_dir,print_name,vis,lw,cmapAge)
...
h = pcolorm(geo.lat,geo.lon,Data_stacked_age);
    ...
    cmap = cmapAge;
    %cmap = cbrewer('div','RdYlBu',end_day);
    colormap((cmap));
    hb=colorbar;
    ...
    ax.Visible = 'off';
    title('Data Age')
    hText = text(0.01,1.0,['Data Age: Date: ',datestr(Date,'dd.mm.yyyy'),' / ',num2str(end_day),' day Stack'],'Units','normalized','HorizontalAlignment','left','VerticalAlignment','bottom','FontSize',12);
