...
for a in *_untar; do
  (cd $a && gdalwarp -tr 500 500  *_pixel_qa.tif pixel_qa_500.tif && gdalwarp -t_srs EPSG:4326 pixel_qa_500.tif pixel_qa_500_wgs.tif && gdal_merge.py -co "PHOTOMETRIC=RGB" -separate *_band{4,3,2}.tif -o L08_rgb.tif && gdal_pansharpen.py *_band4.tif L08_rgb.tif L08_B4_pan_rgb.tif -r bilinear -co COMPRESS=DEFLATE -co PHOTOMETRIC=RGB);
done
